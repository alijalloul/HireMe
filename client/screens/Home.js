...
const Home = ({ navigation }) => {
  ...
  const [numberOfFilters, setNumberOfFilters] =
    useState(0);
  ...
  const page = useSelector(
    (state) => state.jobPosts.currentPage
  );
  const numberOfPages = useSelector(
    (state) => state.jobPosts.numberOfPages
  );
  ...
  useEffect(() => {
    const setItem = async () => {
      try {
        ...
        console.log(
          "async set screenName to hometabs error: ",
          error
        );
      }
    };
